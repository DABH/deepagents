...
class _ThreadREPL:
    ...
    All `ctx.*` operations go through the slot's worker: a dedicated thread
    with the default `ThreadWorker`, the caller's own with `InlineWorker`.
    """
    ...
    def eval_sync(
        ...
        *,
        ...
        if isinstance(self._worker, InlineWorker) and _loop_running_here():
            # An eval suspends; refuse before any JS runs rather than orphan
            # host tasks on the caller's loop.
            msg = (
...
class _Slot:
    ...
    The worker is a dedicated OS thread by default (`ThreadWorker`), so one
    conversation's slow JS can't block others; with `execution="inline"` it
    is the caller's own thread and loop.
    """
